...
% 1.) J(θ)=  1/m * SUM(i=1 to m) * SUM(k=1 to K) [−y(i)log((hθ(x(i)))k)−(1−y(i))log(1−(hθ(x(i)))k)] 
% regularized
% 2.) λ/(2*m) * [[SUM(j=1 to 25) * SUM(k-1 to 400) * (Theta1^2] + [SUM(j=1 to 25) * SUM(k-1 to 400) * (Theta2^2]]]
...
totalDelta1 = zeros(size(Theta1));
totalDelta2 = zeros(size(Theta2));
...
for i = 1:m
	...
	totalDelta1 = totalDelta1 + delta2(2:end)' * a1;
	totalDelta2 = totalDelta2 + delta3' * a2;
end;

Theta1_grad = totalDelta1 / m;
Theta2_grad = totalDelta2 / m;
